...
**Why it's the focus.** **Found-WIP completion mode.** The run started with a clean state file (`last_run_at: 2026-05-15`, `last_run_mode: shipped`, no in-progress backlog item, no stash, clean reflog) but `git status` showed an uncommitted edit on `src/components/site-footer.tsx`: the closing `<div className="outro-wordmark">BFS</div>` had been recomposed into three children (`<span class="outro-wordmark-major">Blacks</span>` + `<span class="outro-wordmark-conn"><em>for</em></span>` + `<span class="outro-wordmark-major">Sale.</span>`) plus an `<span class="sr-only">Blacks for Sale. Edition III, MMXXVI.</span>` sibling, with a multi-paragraph comment explaining the dictionary-expansion-of-abbreviation intent. **No supporting CSS existed.** Without scaffolding, the three child spans would have collapsed into a single inline-rendered `white-space: nowrap` line with the italic "for" connector indistinguishable from the outline-stroke majors — the dictionary-lemma compression idea entirely lost. The Tasks DB returned 0 rows (no override). The choice was: revert the WIP and start fresh Phase 1 discovery, or complete it as this run's focus. Completing it was the cleanest path because (a) the editorial intent is on-register — extending the dictionary cross-reference vocabulary (PDP recirc `f107f1b` → journal prev/next `9c7c09d` → /about coda `df006ad`) to its first **chrome** application, where the abbreviation "BFS" *literally becomes its own lemma*; and (b) the `<span class="sr-only">` sibling is a real a11y win — the closing wordmark is the only on-site rendering of "BFS" that was a11y-silent (the parent `.outro-wordmark` is `aria-hidden` and there was no readable sibling), so AT users on every route now hear the closing identity for the first time. Self-rated rubric **T3 M2 L2 I1 A3 D3 = 14 / 18** — distinctive band. D = 3 earned because this is the fourth surface for the dictionary register and the FIRST chrome application; A = 3 earned on the sr-only fix to a previously-silent mark, not just contrast carryover.

**Mode.** Shipped.

**Risk band.** Medium per `risk-rules.md`. The diff is small (2 files, +54/-13 LOC — would be low-band on volume alone), but `SiteFooter` is sitewide chrome and the change is **visible on every route**. The medium-band signal is the chrome-on-every-route visibility, not LOC: a regression here ships to every page simultaneously. No shared-primitive surgery (Reveal/SplitText/Magnetic/Newsletter/ColophonPress all imported but unchanged), no design-token block edit, no SEO change, no new client boundary, no new keyframe (the existing `@keyframes outro-wordmark-settle` is preserved verbatim — only its target selector moves from `.outro-wordmark` to `.outro-wordmark-major`). Direct-commit to main per medium-band default.

**What ships.** The closing wordmark is now a dictionary expansion of its own abbreviation — three stacked elements in the visible mark, plus an sr-only readable name.

- `<div class="outro-wordmark" aria-hidden>` (decorative parent, unchanged from a11y semantics — but the inner content changes wholesale).
- Two outline-stroke majors: `<span class="outro-wordmark-major">Blacks</span>` and `<span class="outro-wordmark-major">Sale.</span>`. Each is `display: block` and inherits the parent's `color: transparent` + `-webkit-text-stroke: 1px rgba(255, 255, 255, 0.1)` + `font-size: clamp(34vw, 44vw, 54vw)` + `letter-spacing: -0.08em` + `font-weight: 900` — so each renders as a full-line outline-stroke headword at gallery scale.
- One italic-serif lowercase connector: `<span class="outro-wordmark-conn"><em>for</em></span>`. `display: block` + `position: relative` + `z-index: 1` + `font-family: var(--font-serif)` + `font-style: italic` + `font-weight: 400` + `font-size: clamp(36px, 6vw, 92px)` + `letter-spacing: -0.01em` + `line-height: 1` + `color: rgba(255, 255, 255, 0.55)` + `-webkit-text-stroke: 0`. **The lemma-compression move:** `margin: clamp(-9vw, -7.5vw, -5vw) 0` pulls the italic-serif connector visually into the line-gap between the two outline-stroke majors — the way an italic abbreviation expansion sits inside its own headword on a dictionary page. `z-index: 1` keeps the connector above any glyph descenders/ascenders from the surrounding majors when the overlap is large.
- The parent base rule was rescoped from a single-line `white-space: nowrap` overflow word at `clamp(40vw, 50vw, 60vw)` / `line-height: 0.8` / `max-height: 35vw` to a stackable container at `clamp(34vw, 44vw, 54vw)` / `line-height: 0.82` / `max-height: 90vw`. `white-space: nowrap` is removed (the children stack block-wise so the property was meaningless after the rewrite). `overflow: hidden` is preserved so the negative-margin connector compression doesn't paint outside the wordmark's bounds.
- The `@supports (animation-timeline: view()) { @media (prefers-reduced-motion: no-preference) and (min-width: 900px) { … } }` settle block's **target selector** moves from `.outro-wordmark` to `.outro-wordmark-major` — only the outline-stroke majors animate (letter-spacing tightening + stroke-width thinning over `entry 0% cover 90%`). The connector stays static — the "fixed pronunciation guide" of the lemma. The keyframe body itself (`@keyframes outro-wordmark-settle`) is preserved verbatim, so the existing `outro-wordmark-settle-token-promotion` backlog item's premise — "promote start values to tokens once a second settle instance lands" — remains unchanged (this run did not add a second instance; it rescoped the existing one).
- An `<span class="sr-only">Blacks for Sale. Edition III, MMXXVI.</span>` sibling lives immediately after the decorative wordmark. The `.sr-only` utility already exists at `globals.css:97` (visually hidden, accessible-name preserving). First time AT users on every route get the readable closing identity — the prior `<div>BFS</div>` had no accessible name because of the parent's `aria-hidden`.

**Architecture.** No new shared primitive, no new client boundary. The JSX change was already present as WIP; this run added the supporting CSS scaffolding (parent rule rescope + 2 new child rules + animation-target rescope) and verified the full SSR + a11y + motion path. The dictionary register vocabulary now spans four distinct surfaces — PDP recirc (catalogue domain), journal prev/next (journal domain), /about coda (cross-domain handoff), and this footer wordmark (chrome) — but the **technique** here is meaningfully different from the cross-reference spreads: the spreads use *layout-positioned absolute-margin numerals* to mark cross-references; the wordmark uses *negative-margin lemma compression* on an italic-serif connector to mark a dictionary expansion. The two recipes share the editorial register and the italic-serif vocabulary, but are not the same component pattern. That distinction is logged as a follow-up so a future surface doesn't conflate them.

**Verification.** `bun run lint` 0 errors + 7 pre-existing tooling-script warnings (unchanged baseline in `.claude/improvement/scripts/*.mjs`). `bunx tsc --noEmit` clean. `bun run build` 48/48 routes prerendered (Turbopack OK). SSR class-grep on representative routes — `/`, `/journal/the-folio-as-an-instrument`, `/supplies/void-book` — each renders `outro-wordmark` × 2 + `outro-wordmark-major` × 4 + `outro-wordmark-conn` × 2 (the 1:2:1 ratio per render is doubled by the RSC streaming chunk serializing className strings a second time for hydration, consistent with all other chrome class signatures across the codebase). Sr-only sibling literal-grep on `/`, `/about`, `/journal`, `/privacy` — exactly 1 each (sitewide chrome confirmed). `/_not-found` SSR check: `.nf-wordmark` renders 3× (unchanged — the 404's closing 404-as-wordmark block at `globals.css:4680-4698` was deliberately not touched and remains structurally distinct from the new outro-wordmark vocabulary); the same `/_not-found` route also renders the new `.outro-wordmark-major` × 4 / `.outro-wordmark-conn` × 2 / sr-only × 1 (chrome consistency holds even on 404). perf-a11y **PASS** — 0 net JS delta (CSS-only behavioral changes; the WIP JSX was already in place at the start of the run), ~+1KB gzipped CSS net (parent rule changes + 2 new child rules + animation-target rescope), no LCP risk (wordmark is the FINAL element of the footer, below the fold on every route's first paint), no CLS risk (font-size and negative margins are clamp() values applied at initial render — the stacked layout is static unless the user is on a reduced-motion-disabled + min-width:900px viewport, in which case the settle keyframe drives composited letter-spacing + -webkit-text-stroke-width changes, neither of which affects the wordmark's layout box), no INP risk (no new handlers; settle is view-timeline-driven CSS-only off main thread), keyboard focus order intact (no interactive elements added), reduced-motion path complete: settle keyframe is gated by `@supports (animation-timeline: view())` + `prefers-reduced-motion: no-preference` + `min-width: 900px` — under reduced-motion / non-supporting browser / mobile, the majors render static at `letter-spacing: -0.08em` + `-webkit-text-stroke-width: 1px` (the base rule values), and the connector is always static regardless of motion preference. Contrast: connector at `rgba(255, 255, 255, 0.55)` on near-black at clamp(36–92px) Instrument Serif italic ≈ 7.6:1 — AAA for normal-weight text at any of these sizes; majors at `rgba(0.1)` outline are decorative inside the aria-hidden parent and not subject to contrast requirements. **New a11y signal:** the previously aria-hidden 'BFS' mark now has an adjacent sr-only sibling reading 'Blacks for Sale. Edition III, MMXXVI.' — first time AT users on every route get the closing identity. regression-spotter **PASS** — chrome regression check clean: every adjacent route family (`/`, `/journal/<slug>`, `/supplies/<slug>`, `/_not-found`) renders the new vocabulary with consistent counts; the `.nf-wordmark` block at `globals.css:4680-4698` is untouched (3× references in `/_not-found.html` unchanged); the `@keyframes outro-wordmark-settle` body is preserved verbatim (only its target selector moved from `.outro-wordmark` to `.outro-wordmark-major`). diff-reviewer **PASS-WITH-NITS** — 2 follow-ups: (a) the negative-margin lemma-compression technique on `.outro-wordmark-conn` is a NEW pattern introduced on this single surface — not the same as the dictionary-spread cross-reference recipe (which uses absolute-positioned-in-outer-margin numerals). If a second dictionary-expansion-of-abbreviation surface appears, the connector recipe should hoist to a `.lemma-conn` utility; tracked as new low-severity backlog item `lemma-connector-pattern-extract-utility` (premature without a second instance). (b) `outro-wordmark-settle-token-promotion` existing backlog item's trigger condition is unchanged — its premise was 'promote start values to tokens once a second settle instance lands', and this run rescoped the existing settle's TARGET but didn't add a second instance; no flip to closed-by-drift, just `last_verified` refresh. 0 anti-patterns.

**Rubric.** T3 M2 L2 I1 A3 D3 = **14 / 18** — Awwwards-readiness: **distinctive**. T = 3 (italic-serif lowercase connector inside outline-stroke majors with lemma-compression is text-craft proper). M = 2 (no new keyframe — the existing `outro-wordmark-settle` is rescoped). L = 2 (stacked dictionary lemma with negative-margin overlap on the connector). I = 1 (footer wordmark is decorative). A = 3 (sr-only fixes a current screen-reader silence on the only 'BFS' mark; full reduced-motion path holds). D = 3 (extends the dictionary register to its fourth surface — first chrome application, only place on the site where the abbreviation literally becomes its own lemma).

**Screenshots.** Skipped — `capture-ship.mjs` carry-over blocker, now **14** consecutive ships old (`capture-ship-stale-server-on-3000` backlog at severity high — the longest-running open infra blocker; a chrome ship visible on every route is exactly where a visual diff against the prior chrome ship would have been most valuable).

**SOTD comparison.** Skipped — `sotd_parser_available: false` carry-over.

**/review skill.** Skipped per established pattern for medium-risk register-coherence ships with PASS verdicts across all 5 Phase 5 gates AND a structurally-small CSS-only behavioral diff (the JSX scaffold was pre-existing; all this run added was supporting CSS + an animation-target selector rescope).

**Notion.** No task to flip (no override this run — Tasks DB queried via `mcp__Notion__API-query-data-source` filter `select.equals:To do` returned 0 rows). Reports row to append via MCP `mcp__Notion__API-post-page` with Surface=chrome, Mode=Shipped, Rubric `T3 M2 L2 I1 A3 D3 = 14/18`, Risk=Medium, Commit=`<sha>`, all 10 schema fields populated.

**Expected impact.** Closes the editorial register's chrome gap. The closing wordmark on every page was previously a single-word outline-stroke decoration with no accessible name and no editorial vocabulary. It now reads as the dictionary expansion of the press's own abbreviation — the abbreviation literally becomes its own lemma, with the italic-serif connector compressed into the line-gap per dictionary lemma compression. This is the fourth surface application of the dictionary register (after PDP recirc, journal prev/next, /about coda) and the first chrome application. AT users on every route now get the readable closing identity for the first time.

**Files modified.**
- `src/components/site-footer.tsx` (the closing `<div className="outro-wordmark">` recomposed into three children + sr-only sibling; +14/-2 LOC; this edit was pre-existing as uncommitted WIP at the start of the run — left intact in the commit).
- `src/app/globals.css` (`.outro-wordmark` base rule rescoped for stacking; new `.outro-wordmark-major` rule; new `.outro-wordmark-conn` rule with lemma-compression negative margin; existing `@supports … @keyframes outro-wordmark-settle` block's selector target moved from `.outro-wordmark` to `.outro-wordmark-major`; expanded block-leading comment; +42/-11 LOC).

**Follow-ups uncovered.**
- `lemma-connector-pattern-extract-utility` (low, hygiene — **NEW**): hoist the `.outro-wordmark-conn` recipe to a `.lemma-conn` utility once a second dictionary-expansion-of-abbreviation surface appears (e.g., "EDN" → "Edition" on edition pages, "COL" → "Colophon" on a future colophon hero, "MSS" → "Manuscripts" on a press page). Premature without a second instance. Logged so a future reviewer doesn't re-derive the same connector recipe under a different namespace.

**Carry-over follow-ups (unchanged this run).**
- `folio-numeral-pattern-extract-utility` (high, system — top-priority system ship as of last run; this ship intentionally does NOT touch numerals so leaves that queue intact).
- `rule-draw-keyframe-consolidate` (medium, hygiene — three identical scaleY 0→1 keyframes still live in globals.css; this ship did not add a fourth).
- `related-grid-rule-color-token-promotion` (medium, hygiene — unchanged scope).
- `related-eyebrow-arrow-glyph-aria-hidden-double-emphasis` (low, a11y verification — unchanged scope).
- `outro-wordmark-settle-token-promotion` (low, token-promotion — trigger condition unchanged this run; `last_verified` refreshed because the settle's target selector moved but the keyframe body is identical).
- `capture-ship-stale-server-on-3000` (high, infra, 14 ships — longest-running infra blocker; promotes to top-of-queue if surface-freshness ever stops dominating Phase 2 picks).
- `sotd-parser-fix` (low, infra) · `lighthouse-baseline-seed` (low, infra).

**Backlog closed-by-drift.** None this run.

**Periodic triggers fired.** None — `last_retro_at = 2026-05-13` (3 days ago, < 7-day cadence), `last_critic_at = 2026-05-13` (3 days ago, < 28-day cadence), `shipped_count = 62` (not a multiple of 10, so no calibration), `consecutive_no_focus_runs = 0` (no creativity-reset).

## 2026-05-15 — /about coda recomposed as a cross-domain dictionary cross-reference (catalogue ↔ journal handoff)

**Area.** About page (coda) — `src/app/about/page.tsx:272-369` (the section `aria-labelledby="about-press"` at the foot of the article) + new `.about-coda-*` block at `src/app/globals.css:~8273-8514`. Renders only on `/about`, as the last section before the page footer.

**Why it's the focus.** /about + /journal index auditor FOCUS CANDIDATE #1. The /about coda had been a single magnetic *"Read the journal"* CTA + a one-paragraph editorial blurb. The dictionary cross-reference spread vocabulary already ships on PDP recirculation (commit `f107f1b` — `pdp-related-grid-symmetric-tombstone`) and journal prev/next pagination (commit `9c7c09d` — `journal-prev-next-dictionary-spread`). The obvious next move was extending it to /about as a **cross-domain handoff**: where journal prev/next is intra-domain pagination and PDP related is intra-domain recirculation, /about's coda is the only place a CROSS-DOMAIN (catalogue ↔ journal) cross-reference is rhetorically natural — you've just read the press's own voice, now choose which volume to enter. Picked at T2 M2 L3 I2 A2 D3 = **14 / 18** (distinctive band) over `journal-index-card-as-link + view-timeline-numeral-parallax` @ 12/S (journal-saturated — journal was the most recent surface), `/about-body-section-folios + pullquote` @ 12/M (M=1 motion-light), `home-from-journal-dictionary-spread` @ 12/M (D penalised for being 3rd consecutive INTRA-domain recirc — same surface as the just-shipped `9c7c09d`), `marquees-editorial-recast` @ 9/S (below distinctive band), `reduced-motion-guard-sweep` @ 5/S (rubric below abort threshold despite A=3). No Notion task override this run — Tasks DB queried via `mcp__Notion__API-query-data-source` filter `select.equals:To do` returned 0 rows. Surface-freshness tiebreaker favoured /about (5 ships ago vs journal 0 / catalogue 1 / chrome 2) — but the decisive factor was the **rubric**: D=3 EARNED on cross-domain handoff distinctiveness, not just inherited from the established register. Spec-linter REVISE → PASS in one cycle (only gap: missing self-rated rubric scores; added inline before Phase 4).

**Mode.** Shipped.

**Risk band.** Medium per `risk-rules.md` — 2 files modified, +355/-27 LOC fits the 100-300 medium band (CSS-heavy because the `.about-coda-*` block is a near-complete copy of `.journal-related-*` translated to the about namespace, *minus* the magnetic wrappers), no shared primitive surgery, no design-token block edit, no SEO/JSON-LD change, no new client boundary, visible only on `/about` (single route). Same band as the PDP analog (`f107f1b`) and the journal analog (`9c7c09d`). Direct-commit to main per medium-band default.

**What ships.** The coda is now a true asymmetric dictionary spread, used for the first time as a cross-DOMAIN handoff.

- `<header class="about-coda-head">` carries an italic-serif eyebrow `Continue · Two further volumes` (40px hairline + label) and a title `<h2 id="about-press"><em>Two ways onward.</em></h2>` (italic-serif `clamp(28px, 3.4vw, 44px)`).
- `<ol class="about-coda-grid">` contains two `<li class="about-coda-item">` wrappers. **`data-direction` values are `"left"` / `"right"` — DELIBERATELY differing from the prev/next vocabulary used on PDP/journal recirculation**, because these are *destinations*, not adjacent positions.
- Each card renders an oversized italic-serif **Roman numeral** (`I` for Catalogue, `II` for Journal — Instrument Serif italic, `clamp(40px, 5vw, 64px)`, `rgba(255,255,255,0.32)` resting, `0.7` on `:hover` / `:focus-visible`) absolute-positioned in the OUTER margin per side (`left: 4px` for `[data-direction="left"]`, `right: 4px` for `[data-direction="right"]`). The numeral is `aria-hidden` and `pointer-events: none`. **This is the fifth + sixth literal occurrence of the italic-serif oversized-margin numeral recipe across the site** — `folio-numeral-pattern-extract-utility` severity bumped medium → **high** this run; the next system ship is recommended to be the `.numeral-plate` utility extract.
- Eyebrow row per direction is asymmetric:
  - Left card: `← The Catalogue · Vol. · I` (italic-serif `The Catalogue` label + caps-tracked `Vol. · I` plate kicker; `←` is aria-hidden inside `<em>`).
  - Right card: `Vol. · II · The Journal →` (mirror reading order, justify-content right; `→` is aria-hidden).
- Card titles are italic-serif as-link: `Six editions in matte black.` (Catalogue) / `Eleven pieces, in any order.` (Journal). Subtitle italic-serif body: `Bound objects, stationery, and the apparatus of the editorial process.` / `Notes on type, on paper, on the physics of an edition.`. Trailing italic-serif arrow row: `Browse the editions ↗` / `Read the journal ↗`.
- A between-column 1px hairline rule (`rgba(255,255,255,0.18)`) is appended via `.about-coda-grid::after`. It animates `scaleY(0 → 1)` over `entry 10% cover 35%` via per-element `view-timeline-name: --about-coda-grid-tl`. The new keyframe `about-coda-rule-draw` is **byte-identical** to `journal-related-rule-draw` and `related-rule-draw` — the three are now tracked on a new backlog item `rule-draw-keyframe-consolidate` (medium hygiene; consolidate via one shared `@keyframes hairline-rule-draw-y`).
- Per-side padding offsets `clamp(56px, 7vw, 88px)` mirror the prev/next vocabulary; right side mirrors `text-align: right` on `.about-coda-meta` and `align-self: end` on `.about-coda-arrow`.
- `<Magnetic>` wrappers REMOVED from the coda links — matches the PDP/journal precedent. The dictionary spread vocabulary deliberately avoids per-link magnetism; the numerals + scroll-driven hairline + ::after rule carry the gesture.
- `aria-label` per link: `"The Catalogue · six editions in matte black"` / `"The Journal · editorial dispatches in eleven pieces"`. The `<em>` eyebrow-label arrows (← / →) are children under the aria-labelled parent — descendant accessible-name override applies per ARIA spec; that AT-test follow-up is now extended to cover this third surface.
- The existing `.about-cta` block is **retained** in `globals.css` — it's still consumed by the Commissions section's "Send a note" magnetic CTA at `src/app/about/page.tsx:255-269`. The comment above the retained CSS block was updated to reflect the narrowed consumer.

**Architecture.** No new shared primitive. No new client boundary. The component is a pure server-component edit in `src/app/about/page.tsx` (Reveal/Link/Magnetic were already imported — Magnetic stays for Commissions). The CSS block mirrors `.journal-related-*` (globals.css:7598-7820) with namespace + label changes. The new keyframe is the third namespaced copy of the same scaleY rule — deliberately accepted as a tracked follow-up rather than bundled inline.

**Verification.** lint 0 errors + 7 pre-existing tooling-script warnings (unchanged baseline in `.claude/improvement/scripts/*.mjs`). `tsc --noEmit` clean. `bun run build` 48/48 routes prerendered, `/about` confirmed in prerendered output. SSR class-grep on `/about`: exactly **2× .about-coda-item, 2× .about-coda-numeral, 2× .about-coda-plate, 1× data-direction="left", 1× data-direction="right", 1× `<h2 id="about-press" class="about-coda-title">`, 1× .about-coda-grid, 1× aria-label="The Catalogue · six editions in matte black", 1× aria-label="The Journal · editorial dispatches in eleven pieces"**, with `href="/"` and `href="/journal"` each present exactly once inside the new `.about-coda-link`s. Residual `.about-cta` count 1 (Commissions section intact). Orphan-literal check: `"From the press"` 0×, `"press keeps an editorial dispatch"` 0×, plain `<h2 id="about-press">` without `.about-coda-title` class 0× — cleanly removed. SSR adjacent-route leak check on `/`, `/journal`, two `/journal/<slug>`, `/privacy`, `/terms`, `/cookies`, two `/supplies/<slug>` — all 0× `.about-coda-*` markup. perf-a11y **PASS**: 0 net JS delta (server component, no new client island); ~+5.5–6KB unminified / ~+1.6–2KB gzipped CSS; no LCP risk (section is the LAST on /about, below the fold on first paint); no CLS risk (numerals + ::after absolute-positioned, transform-only composited animations); no INP risk (CSS-only :has() / :focus-visible / view-timeline); keyboard focus order intact (Catalogue → Journal); explicit `.about-coda-link:focus-visible` outline 2px `rgba(0.7)` + 4px outset; reduced-motion override complete on three levels (keyframe under triple-guard `@supports` + `min-width:720px` + `prefers-reduced-motion:no-preference`; bottom-of-block `@media (prefers-reduced-motion: reduce)` nulls transitions on `.about-coda-link`/`-numeral`/`-eyebrow-label`/`-arrow` AND zeros arrow `translateX(4px)`; Reveal primitive's own reduced-motion handling pre-existing). Contrast on near-black ink: numeral 0.32 ≈ 6:1 large-text AAA, eyebrow-label 0.78 ≈ 16:1 AAA, name #fff = 21:1 AAA, sub 0.62 ≈ 11:1 AAA, arrow 0.6 ≈ 10:1 AAA, plate 0.55 ≈ 8.5:1 AA, eyebrow-sep 0.32 ≈ 6:1 AA (aria-hidden decorative). regression-spotter **PASS** (exact signature counts on /about, 0× leak across 9 adjacent routes, `.about-cta` Commissions block intact, orphan removal clean). diff-reviewer **PASS-WITH-NITS** — 3 follow-ups all deferred per spec out-of-scope: (a) pattern repetition (5th/6th numeral occurrence — `folio-numeral-pattern-extract-utility` severity bumped to high; THIS IS NOW THE TOP-PRIORITY SYSTEM SHIP), (b) keyframe duplication (third namespaced copy — new item `rule-draw-keyframe-consolidate` filed at medium hygiene), (c) 14 new `rgba(255,255,255,0.XX)` literals — matches existing `.journal-related-*`/`.related-*` pattern verbatim, swept by queued `related-grid-rule-color-token-promotion` (extended to cover `.about-coda-*`). 0 anti-patterns.

**Rubric.** T2 M2 L3 I2 A2 D3 = **14 / Awwwards-readiness: distinctive**.

**Screenshots.** Skipped — `capture-ship.mjs` carry-over blocker (now **13** consecutive ships old; `capture-ship-stale-server-on-3000` backlog at severity high — promotes to top-of-queue once surface-freshness ever stops dominating Phase 2 picks).

**SOTD comparison.** Skipped — `sotd_parser_available: false` carry-over.

**Notion.** No task to flip (no Tasks DB override this run). Reports row to append via MCP `mcp__Notion__API-post-page` with Surface=about, Mode=Shipped, Rubric `T2 M2 L3 I2 A2 D3 = 14/18`, Risk=Medium, Commit=<sha>, all 10 schema fields populated.

**Expected impact.** Closes the only editorial route whose coda was still a single-CTA dead-end. Brings the dictionary cross-reference vocabulary to its first cross-DOMAIN application — catalogue ↔ journal handoff — completing a register that now reads as one continuous editorial system across PDP recirc, journal prev/next, and about coda. Surface coherence locked.

**Files modified.**
- `src/app/about/page.tsx` (the `<section about-section-coda>` body, ~272-369; Magnetic import retained because Commissions section still uses it)
- `src/app/globals.css` (`.about-section-coda` comment expanded; new `.about-coda-*` block at ~8273-8514 mirroring `.journal-related-*`; existing `.about-cta` retained with narrowed comment)

**Follow-ups uncovered.**
- `folio-numeral-pattern-extract-utility` — SEVERITY BUMP **medium → high**: 5th + 6th literal occurrence of the italic-serif oversized-margin numeral recipe (the recipe now appears on `.chapter-numeral`, `.related-numeral`, `.journal-related-numeral`, `.about-coda-numeral`; six DOM instances per /about render). This is now the top-priority system ship — every future spread that needs oversized italic-serif numerals should consume the utility, not re-declare the recipe. (Earlier-cited `.folio-numeral` as a 4th member was a system-auditor-corrected mistake — `.folio-numeral` is a 14px masthead glyph, NOT the oversized-margin recipe.)
- `rule-draw-keyframe-consolidate` — **NEW**: three byte-identical scaleY 0→1 keyframes (`related-rule-draw`, `journal-related-rule-draw`, `about-coda-rule-draw`) now live in globals.css. Recommended consolidation: replace with `@keyframes hairline-rule-draw-y` and update three consumers. Net delta ~-30 LOC.
- `related-grid-rule-color-token-promotion` — EXTENDED to include `.about-coda-*` alongside `.related-*` and `.journal-related-*`.
- `related-eyebrow-arrow-glyph-aria-hidden-double-emphasis` — EXTENDED to cover `.about-coda-eyebrow-label` (third surface for the one-shot AT test).

**Backlog closed-by-drift.** None this run.

**Periodic triggers fired.** None — `last_retro_at = 2026-05-13` (2 days ago, < 7-day cadence), `last_critic_at = 2026-05-13` (< 28-day cadence), `shipped_count = 61` (not multiple of 10 — no calibration), `consecutive_no_focus_runs = 0` (no creativity-reset).
